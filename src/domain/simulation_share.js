...
export function buildSimulationShareUrl({ payload, origin, pathname, searchParam = SIMULATION_SHARE_SEARCH_PARAM } = {}) {
  const encoded = encodeSimulationShare(payload);
  const base = origin || "";
  const path = pathname || "/";
  return `${base}${path}?${encodeURIComponent(searchParam)}=${encodeURIComponent(encoded)}`;
}

export function buildSimulationShareCodeUrl({ code, origin, pathname, searchParam = SIMULATION_SHARE_SEARCH_PARAM } = {}) {
  const normalizedCode = String(code || "");
  const base = origin || "";
  const path = pathname || "/";
  return `${base}${path}?${encodeURIComponent(searchParam)}=${encodeURIComponent(normalizedCode)}`;
}
...
export function serializeSimulationState({ simulation, treeData, origin, pathname, searchParam, dataVersion } = {}) {
  ...
  return {
    ...
    url: buildSimulationShareUrl({ payload, origin, pathname, searchParam })
  };
}
