...
export function evaluateNode(nodeId, state, nodesOrMap) {
  ...
  if (rank >= maxRank) {
    return { id, node, rank, maxRank, isSpecial: false, alwaysVisible, canUnlock: false, canUpgrade: false, reason: "max-rank", nextCost: EMPTY_COST };
  }
  if (rank > 0) {
    return { id, node, rank, maxRank, isSpecial: false, alwaysVisible, canUnlock: false, canUpgrade: true, reason: "upgrade", nextCost: getNodeCost(node, rank + 1) };
  }
...
export function planBatchUnlock(targetNodeId, state, nodesOrMap) {
  ...
  const collect = (id, context) => {
    const { order, visiting, visited, missing } = context;
  ...
  const { order, missing } = context;
  const uniqueOrder = [...new Set(order)];
  const prospectiveRanks = normalizeRanks(state?.ranks);
  uniqueOrder.forEach((id) => {
    if (!prospectiveRanks[id]) prospectiveRanks[id] = 1;
  });
  ...
  const targetRanks = { ...prospectiveRanks };
  delete targetRanks[targetId];
  const targetEvaluation = evaluateNode(targetId, { ...state, ranks: targetRanks }, nodesMap);
...
export function applyBatchUnlock(targetNodeId, state, nodesOrMap) {
  ...
  for (const id of plan.nodeIds) {
    ...
    const toRank = Math.min(getMaxRank(node), fromRank + 1);
    nextRanks[id] = toRank;
    applied.push({
      ok: true,
      id,
