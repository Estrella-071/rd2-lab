...
const DETAILED_STATS_ACTIONS = new Set([
  "SET_GAME_DATA",
  "SET_SIMULATION_MODE",
...
function stripColorSections(value) {
  return String(value ?? "").replaceAll(/<color=[^>]*>[\s\S]*?<\/color>/gi, "");
}
...
export class DetailedStatsView {
  ...
  _formatItem(item) {
    ...
    return stripColorSections(text)
      .replaceAll(/<br\s*\/?>(?=.)/gi, " ")
      .replaceAll(/<\/?[a-z][^>]*>/gi, "")
  ...
  _handleStoreUpdate(state, action) {
    if (!this.isOpen || !DETAILED_STATS_ACTIONS.has(action?.type)) return;
    this.render(state);
  }
  ...
  init() {
    ...
    this._unsubscribe = this.store.subscribe((state, action) => this._handleStoreUpdate(state, action));
  }
