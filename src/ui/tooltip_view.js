...
import { shouldPlaceTooltipBelow, computeTooltipScreenCoordinates } from "../domain/tooltip_position.js";
...
function setSpecialSimulationAction(view, button, node, simEval, batchPlan) {
  if (batchPlan.ok && batchPlan.count > 0) {
    setBatchSimulationAction(view, button, node, batchPlan);
    return;
  }
  button.classList.add("is-disabled", "is-special");
  button.disabled = true;
  const specialLabel = getFactionLevelProgressLabel(node, simEval.currentFactionLevel)
    || view._t("simulation.specialUnlock", {}, "Special unlock condition");
  button.innerHTML = `<span class="sim-btn-text">${escapeHtml(specialLabel)}</span>`;
}

function setRevokeSimulationAction(view, button, node, revokePlan) {
...
function createSimulationActionButton(view, context, simEval, revokePlan, batchPlan) {
  ...
  if (simEval.isSpecial) {
    setSpecialSimulationAction(view, button, node, simEval, batchPlan);
  } else if (simEval.rank > 0) {
    setRevokeSimulationAction(view, button, node, revokePlan);
  } else if (simEval.rank === 0) {
    setUnrankedSimulationAction(view, button, context, simEval, batchPlan);
  }
  return button;
}
...
export class TooltipView {
  ...
  constructor({ store, selectNodeUseCase, navigateViewportUseCase, tooltipElement, nodePositions = new Map(), tagDefinitions = {}, localization }) {
    ...
    this._tooltipBorderLeft = 0;
    this._tooltipBorderRight = 0;
  ...
  init() {
    ...
    this._unsubscribe = this.store.subscribe((state, action) => {
      if (action?.type === ActionTypes.UPDATE_VIEWPORT || action?.type === "SET_VIEWPORT") {
        this._handleViewportUpdate(state);
        return;
      }
      this.render(state);
    });
  ...
  _updateActiveSelection(state, selectedNodeId, selectedNode) {
    ...
    if (this._switchTimer || this._enterTimer || this._closeTimer) {
      ...
      const isVisible = !this.tooltipEl.hidden
        && hasClass(this.tooltipEl, "is-active")
        && !hasClass(this.tooltipEl, "is-closing");
      if (isVisible) this._refreshActiveContent(state, selectedNodeId, selectedNode);
      return true;
    }
    const isActive = !this.tooltipEl.hidden && hasClass(this.tooltipEl, "is-active") && !hasClass(this.tooltipEl, "is-closing");
    if (!isActive) return false;
    this._refreshActiveContent(state, selectedNodeId, selectedNode);
    this._positionTooltip(selectedNodeId, state);
    return true;
  }
  ...
  _readTooltipDimensions(isMobile) {
    if (this._tooltipDimensionsDirty) {
      const height = this.tooltipEl.offsetHeight;
      const width = this.tooltipEl.offsetWidth;
      if (height) this.cachedTipHeight = height;
      if (width) this.cachedTipWidth = width;
      const computedStyle = typeof window !== "undefined" && typeof window.getComputedStyle === "function"
        ...
        : null;
      this._tooltipBorderLeft = Number.parseFloat(computedStyle?.borderLeftWidth || "0") || 0;
      this._tooltipBorderRight = Number.parseFloat(computedStyle?.borderRightWidth || "0") || 0;
      this._tooltipDimensionsDirty = false;
    }
    return {
  ...
  _bindUpgradeButtons(btnRow, node, container) {
    return bindDiceUpgradeButtons(this, btnRow, node, container);
  }
