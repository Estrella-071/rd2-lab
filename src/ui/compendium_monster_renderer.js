...
/**
 * Render monster groups, static poster cards, and rankable SP statistics.
 */
...
export function createMonsterCompactItem(compendium, monster, index = 0) {
  ...
  item.dataset.compendiumCategory = "monster";
  item.dataset.compendiumId = String(monster.id);
  ...
  const posterPath = resolvePublicIconFilename(monster.poster || monster.icon, "monster_static_boss_4.png");
  ...
  item.addEventListener("click", (e) => {
    ...
    compendium.onNavigate?.({ kind: "compendium-card", category: "monster", id: String(monster.id) });
  });
...
export function createMonsterCard(compendium, monster, index = 0) {
  ...
  card.dataset.compendiumCategory = "monster";
  card.dataset.compendiumId = String(monster.id);
  ...
  header.innerHTML = `
    ...
    <div class="tooltip-dice-visual">
      <div class="monster-static-visual">
        <img class="monster-static-poster" src="icons/${posterPath}" alt="${escapeHtml(monster.name_zh || monster.name_en || fallbackName)}" loading="lazy" decoding="async" data-fallback-src="icons/Big_minion.png" />
      </div>
    </div>
  `;
