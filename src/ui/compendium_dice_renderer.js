...
import { appendAwakeningSection, appendDiceStats } from "./dice_details_renderer.js";
...
export function createDiceCard(compendium, node, index = 0) {
  ...
  card.dataset.compendiumCategory = "dice";
  card.dataset.compendiumId = String(node.id);
  ...
  appendAwakeningSection(compendium, body, node, 1);
  appendDiceStats(compendium, body, node);
...
export function createCompactDiceItem(compendium, node, index = 0) {
  ...
  item.dataset.compendiumCategory = "dice";
  item.dataset.compendiumId = String(node.id);
  ...
  item.addEventListener("click", (e) => {
    ...
    compendium.onNavigate?.({ kind: "compendium-card", category: "dice", id: String(node.id) });
  });
