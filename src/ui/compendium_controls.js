...
export function syncCategoryTabs(compendium) {
  const categoryOptions = document.querySelectorAll(".category-option-item");
  ...
  categoryOptions.forEach((option) => {
    const active = (option.dataset.value || "dice") === compendium.category;
    option.classList.toggle("is-selected", active);
    option.setAttribute("aria-pressed", String(active));
    option.setAttribute("aria-selected", String(active));
  });
  const selectedCategory = [...categoryOptions].find((option) => option.classList.contains("is-selected"));
  if (categoryLabel && selectedCategory) categoryLabel.textContent = selectedCategory.textContent.trim();
