...
export class SimulationView {
  constructor({ store, simulationUseCase, container, tooltipElement, localization } = {}) {
    this.store = store;
  ...
  init() {
    ...
    const remoteCode = /^[0-9A-Za-z]{6}$/.exec(searchParams?.get("s") || "")?.[0]
      || /^[0-9A-Za-z]{6}$/.exec(hashParams?.get("s") || "")?.[0]
      || "";
    if (remoteCode) {
      this._importRemoteShare(remoteCode);
    } else if (searchParams?.has("s") || searchParams?.has("sim") || hashParams?.has("s") || hashParams?.has("sim")) {
