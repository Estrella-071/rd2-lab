...
export class SimulationPlanUseCase {
  ...
  serialize({ origin, pathname, searchParam } = {}) {
    const state = this.state;
  ...
  async createShareLink({ origin, pathname, searchParam, serialized = null } = {}) {
    const local = serialized || this.serialize({ origin, pathname, searchParam });
    ...
    return {
      ...
      remote: true,
      code: remote.code,
      url: buildSimulationShareCodeUrl({ code: remote.code, origin, pathname, searchParam })
    };
  }
