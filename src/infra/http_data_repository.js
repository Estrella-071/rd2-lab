...
function assertSafeAssetPath(value, label, url) {
  if (typeof value !== "string" || value.length === 0 || value.length > 240
    ...
    throw new Error(`Invalid monster visual data format received from ${url}: unsafe ${label} path`);
  }
}

function assertMonsterVisualDataShape(data, url) {
  const isRecord = (value) => value && typeof value === "object" && !Array.isArray(value);
  if (!isRecord(data) || !isRecord(data.monsters)) {
    throw new Error(`Invalid monster visual data format received from ${url}`);
  }

  for (const [id, visual] of Object.entries(data.monsters)) {
    if (!id || !isRecord(visual)) {
      throw new Error(`Invalid monster visual data format received from ${url}`);
    }
    if (visual.poster !== undefined) {
      assertSafeAssetPath(visual.poster, "poster", url);
    }
    if (visual.spine !== undefined) {
      if (!isRecord(visual.spine)
        || typeof visual.spine.skeleton !== "string"
        || typeof visual.spine.atlas !== "string"
        || typeof visual.spine.texture !== "string"
        || typeof visual.spine.animation !== "string"
        || visual.spine.animation.length === 0
        || !/^[A-Za-z0-9_.-]+$/.test(visual.spine.animation)) {
        throw new Error(`Invalid monster visual data format received from ${url}`);
      }
      assertSafeAssetPath(visual.spine.skeleton, "skeleton", url);
      assertSafeAssetPath(visual.spine.atlas, "atlas", url);
      assertSafeAssetPath(visual.spine.texture, "texture", url);
    }
  }
}
...
export class HttpDataRepository extends DataRepositoryPort {
  ...
  constructor(options = {}) {
    ...
    this.monsterVisualsUrl = options.monsterVisualsUrl || "monster_visuals.json";
  ...
  async loadBossEvents(url = this.bossEventsUrl) {
    ...
    if (!response.ok && (url === "boss_event_data.json" || url === "data/boss_event_data.json")) {
      ...
      try {
        const fallbackRes = await this.fetchFn(fallbackUrl);
        if (fallbackRes.ok) {
  ...
  /**
   * Load monster visuals JSON data via HTTP.
   * Supports automatic fallback between 'monster_visuals.json' and 'data/monster_visuals.json'.
   * @param {string} [url]
   * @returns {Promise<object>}
   */
  async loadMonsterVisuals(url = this.monsterVisualsUrl) {
    if (this._cache.has(url)) {
      return this._cache.get(url);
    }
    if (!this.fetchFn) {
    ...
    const cacheGeneration = this._cacheGeneration;
    let response = await this.fetchFn(url);
    if (!response.ok && (url === "monster_visuals.json" || url === "data/monster_visuals.json")) {
      const fallbackUrl = url === "monster_visuals.json" ? "data/monster_visuals.json" : "monster_visuals.json";
      try {
        const fallbackRes = await this.fetchFn(fallbackUrl);
        if (fallbackRes.ok) {
          response = fallbackRes;
    ...
    const data = await response.json();
    assertMonsterVisualDataShape(data, url);
    if (cacheGeneration === this._cacheGeneration) this._cache.set(url, data);
    return data;
  }
  ...
  async loadAll(options = {}) {
    const [treeData, svgText, bossEvents, monsterVisuals, metadata, changelog, locales] = await Promise.all([
      ...
      this.loadMonsterVisuals(options.monsterVisualsUrl),
    ...
    return { treeData, svgText, bossEvents, monsterVisuals, metadata, changelog, locales };
  }
