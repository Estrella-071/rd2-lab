import { getNodeMap } from "../domain/simulation_plan.js";
...
async function snapshotRealSvgTreeAsync({ svgElement, targetWidth = 1600, targetHeight = 780 } = {}) {
  ...
  const cssText = collectSnapshotCss(`
    .cost-badge { display: none !important; }
    .node.is-sim-locked:not(.is-selected):not(.is-linked-selected) { opacity: 0.78 !important; filter: none !important; }
    .node.is-sim-locked:not(.is-selected):not(.is-linked-selected) .node-icon,
    .node.is-sim-locked:not(.is-selected):not(.is-linked-selected) .node-icon-flat,
    .node.is-sim-locked:not(.is-selected):not(.is-linked-selected) .node-icon-deep {
      filter: grayscale(1) saturate(0.25) brightness(0.82) !important;
    }
    ...
    path.is-simulation-locked-edge { opacity: 0.12 !important; }
...
export async function generateSimulationShareImage({
  ...
  try {
    ...
    const svgSnapshot = await snapshotRealSvgTreeAsync({
      svgElement,
      targetWidth: treeAreaW,
      targetHeight: treeAreaH
    });
