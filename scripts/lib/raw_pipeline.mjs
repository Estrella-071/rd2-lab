...
function expectationForNode(node, rawNode) {
...
export function buildCanonicalFromRaw(current, raw) {
  ...
  const nodes = raw.tree.nodes.map((rawNode) => {
...
export function buildLineage(raw, canonical, compendium = null) {
  ...
  for (const rawNode of raw.tree.nodes) {
    ...
    expectations[String(rawNode.id)] = expectationForNode(canonicalNode, rawNode);
  }
