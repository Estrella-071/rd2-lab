...
const monsterPosters = JSON.parse(fs.readFileSync(path.join(rootDir, 'site', 'monster_posters.json'), 'utf8'));
const errors = [];
...
if (monsterPosters.snapshot?.snapshot_id !== metadata.canonical?.snapshot_id) errors.push('monster posters snapshot_id does not match canonical metadata');
if (monsterPosters.snapshot?.metadata_ref !== 'data/game_data_metadata.json') errors.push('monster posters metadata_ref does not point to canonical metadata');
if (metadata.canonical?.posters_path !== 'monster_posters.json') errors.push('canonical metadata posters_path is incorrect');
...
if (metadata.source?.raw_tree_edge_count !== data.summary?.edge_count + metadata.source?.topology_correction_count) errors.push('metadata raw/effective topology counts do not reconcile');
