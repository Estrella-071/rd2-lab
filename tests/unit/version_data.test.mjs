...
test("version data: canonical metadata, public snapshots, and generated changelog agree", () => {
  ...
  const visuals = readJson("site/monster_visuals.json");
  ...
  assert.equal(metadata.canonical.public_schema_versions.monster_visuals, 2);
  assert.equal(metadata.canonical.visuals_path, "monster_visuals.json");
  ...
  assert.equal(tree.summary.edge_count, 248);
  assert.equal(compendium.meta.game_data_version, metadata.canonical.game_version);
  assert.equal(visuals.snapshot.snapshot_id, metadata.canonical.snapshot_id);
  assert.equal(visuals.snapshot.metadata_ref, "data/game_data_metadata.json");
