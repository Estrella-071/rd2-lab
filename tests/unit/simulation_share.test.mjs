...
test("simulation share: URL round-trips ranks, team, runes and common nodes", () => {
  ...
  const serialized = serializeSimulationState({ simulation, treeData: { nodes }, origin: "https://example.test" });
  assert.ok(serialized.url.startsWith("https://example.test/simulation/state/"));
...
test("share image render state: unrelated pre-unlocked roots are treated as locked", () => {
  const treeData = {
    nodes: [
      { id: "1001", node_type: "DICE", max_rank: 1 },
      { id: "1005", node_type: "DICE", max_rank: 1 },
      { id: "4008", node_type: "DICE", max_rank: 1, incoming: [] },
      { id: "2", node_type: "DICE", max_rank: 1, incoming: ["1001"] },
      { id: "3", node_type: "DICE_RUNE", max_rank: 3, incoming: ["2"] }
    ]
  };
  const state = computeShareRenderUnlockState({ ranks: { "2": 1 } }, treeData);
  assert.deepEqual([...state.manualIds], ["2"]);
  assert.ok(state.manualPathIds.has("1001"));
  assert.ok(state.renderUnlockedIds.has("1001"));
