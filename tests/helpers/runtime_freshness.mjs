...
import { minifyCssForStaging } from '../../scripts/lib/css_runtime.mjs';
...
function normalizeRuntimeContent(relativePath, content) {
  const text = content.toString('utf8');
  if (relativePath === 'index.html') {
    return Buffer.from(text.replace(/\?v=[a-f0-9]{16}/g, ''), 'utf8');
  }
  if (relativePath.endsWith('.js')) {
    return Buffer.from(text.replace(/(\.\.?\/[^"'?]+\.js)\?v=[a-f0-9]{16}/g, '$1'), 'utf8');
  }
  if (relativePath.endsWith('.css')) {
    return Buffer.from(minifyCssForStaging(text), 'utf8');
  }
  return content;
}
...
function compareFileTrees(sourceRoot, runtimeRoot, relativeFiles, mismatches) {
  ...
  for (const relativePath of sourceFiles) {
    ...
    const sourceContent = normalizeRuntimeContent(relativePath, fs.readFileSync(sourcePath));
    const runtimeContent = normalizeRuntimeContent(relativePath, fs.readFileSync(runtimePath));
    if (!sourceContent.equals(runtimeContent)) {
