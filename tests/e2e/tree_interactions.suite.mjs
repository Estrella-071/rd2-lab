  ...
  try {
    ...
    const blankClickState = await page.evaluate(() => {
      ...
      return {
        ...
        bodyHasPrereqHighlight: document.body.classList.contains('has-prereq-highlight')
      };
    });
    assertEqual(blankClickState.selectedNodeId, null, 'Blank map click should clear the selected node');
    assertEqual(blankClickState.activePrereqCount, 0, 'Blank map click should clear the active prerequisite path');
    assertEqual(blankClickState.showPrereqMode, true, 'Blank map click should preserve prerequisite display mode');
    assert(!blankClickState.bodyHasPrereqHighlight, 'Blank map click should remove prerequisite highlighting');
    await page.evaluate(() => {
      document.getElementById('viewport')?.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }));
    });
    ...
    assertEqual(secondBlankClickState.showPrereqMode, false, 'Second blank map click should exit prerequisite display mode');
    passedAssertions += 6;
    ...
    await page.evaluate(() => window.__TEST_HOOKS__.closeTooltip(true));
    await page.waitForSelector('#tooltip', { state: 'hidden', timeout: 3000 });
    const dragNode = await page.$('g.node[data-node-id="1001"]');
