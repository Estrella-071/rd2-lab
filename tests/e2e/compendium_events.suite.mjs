...
export async function runCompendiumEventsSuite(options = {}) {
  ...
  try {
    ...
    // reopened overlay or dispose its freshly acquired single Spine context.
    ...
    const rapidReopenState = await page.evaluate(() => ({
      ...
      activeContexts: window.RD2App.spineEngine.getActiveContextCount(),
      maxContexts: window.RD2App.spineEngine.maxActiveContexts
    }));
    assert(
      !rapidReopenState.overlayHidden &&
        rapidReopenState.activeContexts <= rapidReopenState.maxContexts,
      'Rapid compendium close/reopen must keep the overlay visible and Spine contexts within the pool cap'
    );
    passedAssertions++;
    ...
    await page.waitForFunction(() => {
      const visual = document.querySelector('#compendium-modal-card-slot .monster-spine-visual');
      return visual?.classList.contains('is-ready') || visual?.classList.contains('is-static');
    }, null, { timeout: 15000 });
    const modalSpineBeforeClose = await page.evaluate(() => ({
      activeContexts: window.RD2App.spineEngine.getActiveContextCount(),
      modalOpen: !document.getElementById('compendium-dice-modal')?.hidden,
      ready: document.querySelector('#compendium-modal-card-slot .monster-spine-visual')?.classList.contains('is-ready'),
      staticFallback: document.querySelector('#compendium-modal-card-slot .monster-spine-visual')?.classList.contains('is-static')
    }));
    assert(modalSpineBeforeClose.modalOpen, 'Monster detail modal should remain open while its visual is available');
    assert(
      modalSpineBeforeClose.ready || modalSpineBeforeClose.staticFallback,
      'Monster detail should expose either a live Spine canvas or its static fallback'
    );
    assertEqual(
      modalSpineBeforeClose.activeContexts,
      modalSpineBeforeClose.ready ? 1 : 0,
      'Monster detail should own one context only when Spine initialization succeeds'
    );
    await page.keyboard.press('Escape');
    await page.waitForFunction(() => (
      document.getElementById('compendium-dice-modal')?.hidden
      && window.RD2App.spineEngine.getActiveContextCount() === 0
    ));
    const modalSpineAfterClose = await page.evaluate(() => ({
      activeContexts: window.RD2App.spineEngine.getActiveContextCount(),
      modalHidden: document.getElementById('compendium-dice-modal')?.hidden
    }));
    assertEqual(modalSpineAfterClose.activeContexts, 0, 'Closing monster detail should leave no active WebGL context');
    assert(modalSpineAfterClose.modalHidden, 'Closing monster detail should hide the compact modal');
    await page.click('.view-toggle-btn[data-mode="cards"]');
