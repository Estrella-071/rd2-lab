  ...
  try {
    ...
    await page.waitForFunction(
      () => document.getElementById('compendium-category-current-label')?.textContent.trim() === '怪物'
        && Boolean(document.querySelector('.compendium-card.is-normal-monster .monster-spine-poster')),
      null,
      { timeout: 10000 }
    );
    const iconSafetyState = await page.$eval('.compendium-card.is-normal-monster .monster-spine-poster', (img) => ({
      src: img.getAttribute('src'),
      onerror: img.getAttribute('onerror') || '',
      injected: Boolean(window.__RD2_ICON_INJECTION__),
    }));
    assertEqual(iconSafetyState.src, 'icons/Boss_Bubble.png', 'Unsafe monster icon must use the reviewed fallback asset');
    ...
    // Check the runtime marker and WebGL context count.
    const runtimeAssertion = await page.evaluate(() => ({
      runtime: window.__RD2_RUNTIME__,
      maxActiveSpineContexts: window.RD2App?.spineEngine?.maxActiveContexts,
    }));
    assertEqual(runtimeAssertion.runtime, 'ready', 'App runtime should be ready');
    assertEqual(runtimeAssertion.maxActiveSpineContexts, 1, 'Spine runtime must enforce a single active WebGL context');
    passedAssertions += 2;
