...
The site loads canonical JSON, SVG, and PNG files through a static HTTP server. `HttpDataRepository` owns data requests and caching. `scripts/build_pages.mjs` selects the public files through `site/runtime-allowlist.json` and writes a manifest for staging checks.
