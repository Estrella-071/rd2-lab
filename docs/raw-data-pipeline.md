...
The generator also copies the raw tree node fields and edges, so topology, costs, node identity, direct attack values, and attack intervals cannot silently diverge from the selected `DiceTreeNodeTable` projection.
